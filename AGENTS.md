...
- **Color contrast gate:** color pairs declared in `src/game/testing/contrastSpecs.ts` (WCAG math in `contrast.ts`) are checked in all four palettes (dark/light × standard/high-contrast) by `contrastSpecs.test.ts`. Ratios are never rounded up. Standard-palette shortfalls the owner accepted live in `ACCEPTED_FAILURES`; every high-contrast pair is gated at AA with no exceptions. Run `pnpm contrast:report` to print every pair's ratio, and add new color pairs to `contrastPairs` when introducing colors.
