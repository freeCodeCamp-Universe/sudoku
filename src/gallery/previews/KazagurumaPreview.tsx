...
function isBoxH(r: number, c: number): boolean {
  return SUB_GRIDS.some(
    ({ originRow, originCol }) =>
      r > originRow && r < originRow + 9 &&
      c >= originCol && c < originCol + 9 &&
      (r - originRow) % 3 === 0
  );
}
...
function mergeAndDraw(
  ...
  for (const [y, segs] of hMap) {
    for (const [x1, x2] of merged(segs)) { ctx.moveTo(x1, y); ctx.lineTo(x2, y); }
  }
  for (const [x, segs] of vMap) {
    for (const [y1, y2] of merged(segs)) { ctx.moveTo(x, y1); ctx.lineTo(x, y2); }
  }
  ctx.stroke();
}
  ...
  const canvasRef = usePreviewCanvas(useCallback((ctx, { width }) => {
    ...
    for (let r = 0; r < CELLS; r += 1) {
      for (let c = 0; c < CELLS; c += 1) {
        ...
        if (mB !== m) addSeg(hBorder, (r + 1) * cell - (r + 1 === CELLS ? hw : 0),   c * cell, (c + 1) * cell);
        if (mL !== m) addSeg(vBorder, c * cell + (c === 0 ? hw : 0),                  r * cell, (r + 1) * cell);
        if (mR !== m) addSeg(vBorder, (c + 1) * cell - (c + 1 === CELLS ? hw : 0),   r * cell, (r + 1) * cell);
      }
    }
