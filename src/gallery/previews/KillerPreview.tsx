...
const CAGES: Cage[] = [
  { cells: [0, 1, 9, 10],   bgDark: '#1a1a30', bgLight: '#d8d8f8', dashDark: '#9870d0', dashLight: '#9870d0', clueCell: 0,  clue: '15' },
  { cells: [2, 3, 4],       bgDark: '#0d2a18', bgLight: '#b8e8cc', dashDark: '#acd157', dashLight: '#48a860', clueCell: 2,  clue: '12' },
  { cells: [11, 12, 20, 21],bgDark: '#251a1a', bgLight: '#f8ddd8', dashDark: '#c07050', dashLight: '#c06040', clueCell: 11, clue: '8'  },
  { cells: [18, 19, 27],    bgDark: '#252018', bgLight: '#e8f0c0', dashDark: '#90b040', dashLight: '#70a020', clueCell: 18, clue: '11' },
];
  ...
  const canvasRef = usePreviewCanvas(useCallback((ctx, { width }) => {
    ...
    for (const cage of CAGES) {
      ...
      for (const idx of cage.cells) {
        ...
        if (!cageSet.has((r - 1) * CELLS + c)) { ctx.moveTo(x, y);     ctx.lineTo(x + w, y); }
        if (!cageSet.has((r + 1) * CELLS + c)) { ctx.moveTo(x, y + h); ctx.lineTo(x + w, y + h); }
        if (!cageSet.has(r * CELLS + (c - 1))) { ctx.moveTo(x, y);     ctx.lineTo(x, y + h); }
        if (!cageSet.has(r * CELLS + (c + 1))) { ctx.moveTo(x + w, y); ctx.lineTo(x + w, y + h); }
      }
      ctx.stroke();
    }
    ctx.setLineDash([]);
