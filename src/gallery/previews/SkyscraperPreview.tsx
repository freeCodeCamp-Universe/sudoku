  ...
  const canvasRef = usePreviewCanvas(
    useCallback(
      (ctx, { width }) => {
        ...
        [2, 3, 1, 4, 2, 3].forEach((value, index) =>
          ctx.fillText(String(value), offset + index * cell + cell / 2, offset / 2)
        );
        [3, 2, 4, 1, 3, 2].forEach((value, index) =>
          ctx.fillText(
            String(value),
            offset + index * cell + cell / 2,
            offset + 6 * cell + offset / 2
          )
        );
        [2, 4, 1, 3, 2, 3].forEach((value, index) =>
          ctx.fillText(String(value), offset / 2, offset + index * cell + cell / 2)
        );
        [3, 1, 4, 2, 3, 2].forEach((value, index) =>
          ctx.fillText(
            String(value),
            offset + 6 * cell + offset / 2,
            offset + index * cell + cell / 2
