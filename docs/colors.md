# Board Colors

_Generated from `src/app/theme.css` and `src/game/testing/colorSpecs.ts`. Run `pnpm docs:colors` to regenerate._

## Tokens

| Token | Dark | Light | Dark HC | Light HC |
| --- | --- | --- | --- | --- |
| `--bg-primary` | #0a0a23 | #f5f5f0 | #0a0a23 | #f5f5f0 |
| `--bg-secondary` | #1b1b32 | #ebebe6 | #1b1b32 | #ebebe6 |
| `--bg-surface` | #2a2a40 | #e0e0db | #2a2a40 | #e0e0db |
| `--border` | #3b3b4f | #c8c8d8 | #9898b0 | #55556d |
| `--text-muted` | #858591 | #626262 | #d0d0d5 | #2a2a40 |
| `--text-subtle` | #d0d0d5 | #2a2a40 | #ffffff | #0a0a23 |
| `--text-primary` | #f5f6f7 | #1a1a2e | #f5f6f7 | #1a1a2e |
| `--text-bright` | #ffffff | #0a0a23 | #ffffff | #0a0a23 |
| `--accent-yellow` | #f1be32 | #7b5e2c | #ffeaa8 | #332200 |
| `--accent-blue` | #99c9ff | #1565c0 | #d8ecff | #0a2540 |
| `--board-clue-text` | #99c9ff | #0d47a1 | #99c9ff | #0d47a1 |
| `--color-1` | #e03535 | #e03535 | #ff4d4d | #720000 |
| `--color-2` | #f07820 | #f07820 | #fe8b1a | #a45300 |
| `--color-3` | #d4a828 | #d4a828 | #ffee00 | #918700 |
| `--color-4` | #33a850 | #33a850 | #65e165 | #008a00 |
| `--color-5` | #1aabaa | #1aabaa | #11acac | #006767 |
| `--color-6` | #3a80e0 | #3a80e0 | #4080e8 | #001cd1 |
| `--color-7` | #8e52e8 | #8e52e8 | #9447ff | #380070 |
| `--color-8` | #d94080 | #d94080 | #ff98cc | #950077 |
| `--color-9` | #9898b0 | #9898b0 | #d9d9d9 | #939393 |
| `--accent-green` | #acd157 | #00471b | #ddf2a3 | #092809 |
| `--accent-red` | #ffadad | #850000 | #fff1f1 | #4d0000 |
| `--focus-ring` | #198eee | #198eee | #198eee | #198eee |
| `--cell-border` | #4a4a5e | #d8d8e8 | #cccce0 | #3c3c50 |
| `--cell-bg-light` | #ffffff | #ffffff | #ffffff | #ffffff |
| `--cell-text-light` | #2c5f8a | #2c5f8a | #2c5f8a | #0a2540 |
| `--box-boundary` | #4a4a5e | #b8b8cc | #ffffff | #0a0a23 |
| `--board-frame` | #9898b8 | #8080a8 | #ffffff | #0a0a23 |
| `--cell-diagonal-bg` | #3b3b4f | #e8e8fa | #65658a | #8f8fa8 |
| `--cell-window-bg` | #3b3b4f | #e8e8fa | #65658a | #8f8fa8 |
| `--cell-special-bg` | #3b3b4f | #e8e8fa | #65658a | #8f8fa8 |
| `--cell-even-bg` | #3b3b4f | #e8e8fa | #60607a | #8f8fa8 |
| `--cell-odd-bg` | #12123a | #ffffff | #0f0f2c | #ffffff |
| `--cell-peer-even-bg` | #2c2c4c | #d8d8f0 | #646480 | #8989a2 |
| `--cell-peer-odd-bg` | #1a1a40 | #ededf6 | #0b0b24 | #fafafc |
| `--cell-overlap-2-bg` | #222248 | #eeeef8 | #222248 | #eeeef8 |
| `--cell-overlap-3-bg` | #272751 | #e9e9f6 | #272751 | #e9e9f6 |
| `--cell-overlap-4-bg` | #2c2c5a | #e4e4f4 | #2c2c5a | #e4e4f4 |
| `--cell-overlap-5-bg` | #313163 | #dfdff2 | #313163 | #dfdff2 |
| `--cell-selected-border` | #4a90d9 | #4a90d9 | #a8d4ff | #08306b |
| `--cell-peer-bg` | #272948 | #ededf6 | #272948 | #ededf6 |
| `--cell-peer-structural-bg` | #2e2e44 | #d8d8f0 | #2e2e44 | #d8d8f0 |
| `--cell-same-value-bg` | #1a4f4f | #cdeaea | #1a4f4f | #cdeaea |
| `--cell-error-bg` | #2a1515 | #f8dede | #9c5f5f | #cc7070 |
| `--numpad-chip-label` | rgba(255, 255, 255, 0.9) | #000000 | #000000 | #ffffff |
| `--numpad-chip-label-bright` | rgba(255, 255, 255, 0.9) | #000000 | #000000 | #000000 |
| `--btn-primary-bg` | #f1be32 | #f1be32 | #f1be32 | #6b5300 |
| `--btn-primary-text` | #0a0a23 | #0a0a23 | #0a0a23 | #ffffff |
| `--given-dot` | rgba(255, 255, 255, 0.7) | rgba(0, 0, 0, 0.25) | #ffffff | #0a0a23 |
| `--revealed-dot` | rgba(241, 190, 50, 0.85) | rgba(241, 190, 50, 0.85) | #ffe38a | #493a0f |
| `--candidate-text` | #aaaacc | #555570 | #f0f0fa | #14142e |
| `--overlay-argyle-stroke` | #9898b8 | #8080a8 | #9898b8 | #8080a8 |
| `--overlay-arrow-stroke` | #9898b8 | #9898b8 | #9898b8 | #9898b8 |
| `--overlay-inequality-fill` | #f1be32 | #c79100 | #ffe38a | #a97b00 |
| `--overlay-cage-stroke` | #9898b8 | #5555aa | #9898b8 | #5555aa |
| `--overlay-cage-sum` | #f1be32 | #c79100 | #ffe38a | #8a6a00 |
| `--overlay-jigsaw-stroke` | #9898b8 | #6060a0 | #9898b8 | #6060a0 |
| `--overlay-window-fill` | #3b3b4f | #e8e8fa | #65658a | #8f8fa8 |
| `--overlay-special-fill` | #3a1a6a | #dcc8f4 | #6d6398 | #a087c4 |
| `--overlay-kropki-light` | #f8f8ff | #f8f8ff | #f8f8ff | #f8f8ff |
| `--overlay-kropki-dark` | #4050a0 | #4050a0 | #4050a0 | #4050a0 |
| `--overlay-kropki-ring` | #5060a0 | #5060a0 | #5060a0 | #5060a0 |
| `--overlay-consecutive-fill` | #d0d0e8 | #d0d0e8 | #d0d0e8 | #d0d0e8 |
| `--overlay-consecutive-stroke` | #1b1b32 | #1b1b32 | #1b1b32 | #1b1b32 |
| `--overlay-chain-1` | #99c9ff | #99c9ff | #0064d6 | #3091ff |
| `--overlay-chain-2` | #acd157 | #acd157 | #007a1e | #00a82a |
| `--overlay-chain-3` | #f1be32 | #f1be32 | #866300 | #b98700 |
| `--overlay-chain-4` | #ff9966 | #ff9966 | #a94f00 | #e86c00 |
| `--overlay-chain-5` | #cc88ff | #cc88ff | #9321f6 | #ba69ff |
| `--overlay-chain-6` | #55ddbb | #55ddbb | #007763 | #00a287 |
| `--overlay-chain-7` | #ff88aa | #ff88aa | #bc3265 | #ff498c |
| `--overlay-chain-8` | #88ddff | #88ddff | #007190 | #009dc7 |
| `--overlay-chain-9` | #ffcc55 | #ffcc55 | #726a00 | #9d9200 |
| `--overlay-chain-10` | #dd88cc | #dd88cc | #c100a5 | #f343d9 |
| `--overlay-chain-11` | #88ccaa | #88ccaa | #3f7500 | #57a200 |
| `--overlay-chain-12` | #ffaa66 | #ffaa66 | #d30000 | #ff5252 |

## Per-variant color markers

| Variant | Marker | Token | Kind |
| --- | --- | --- | --- |
| sudoku-x | `data-diagonal` | `--cell-diagonal-bg` | positional |
| windoku | `data-window` | `--cell-window-bg` | positional |
| asterisk | `data-asterisk` | `--cell-special-bg` | positional |
| center-dot | `data-center-dot` | `--cell-special-bg` | positional |
| girandola | `data-girandola` | `--cell-special-bg` | positional |
| even-odd | `data-even` | `--cell-even-bg` | value-derived |
| even-odd | `data-odd` | `--cell-odd-bg` | value-derived |
